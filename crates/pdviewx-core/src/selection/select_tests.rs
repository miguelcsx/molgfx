...
fn geometric_and_secondary_selection_builders_validate_input() {
    assert!(Select::in_sphere(pdviewx_math::Vec3::ZERO, 2.0).is_ok());
    assert!(Select::in_box(-pdviewx_math::Vec3::ONE, pdviewx_math::Vec3::ONE).is_ok());
    assert!(Select::in_sphere(pdviewx_math::Vec3::ZERO, f32::NAN).is_err());
    assert!(Select::in_box(pdviewx_math::Vec3::ONE, -pdviewx_math::Vec3::ONE).is_err());
    assert_eq!(
        Select::helix(),
