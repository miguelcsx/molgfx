...
use pdviewx_math::{Aabb, Bvh, Mat4, SphereBounds};
...
struct StructureAssetData {
    ...
    spatial_bvh: OnceLock<Result<Bvh, pdviewx_math::BvhBuildError>>,
    spatial_bounds: Aabb,
...
fn atom_bounds(atoms: &AtomTable) -> Aabb {
    pdviewx_math::simd::spheres_aabb(atoms.coords().slice(), atoms.radius().values())
}
