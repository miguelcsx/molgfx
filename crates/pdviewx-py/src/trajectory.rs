...
pub(crate) struct PyTrajectoryChunkWindow(pub(crate) pdviewx::TrajectoryChunkWindow);
...
impl PyTrajectoryChunkWindow {
    ...
    fn new(
        ...
        pdviewx::TrajectoryChunkWindow::new(structure.0, start.0, end.0, interpolation)
            .map(Self)
...
#[derive(Clone, Debug)]
pub(crate) struct PyTrajectoryBranch(pdviewx::TrajectoryBranch);
...
impl PyTrajectoryBranch {
    ...
    fn new(from_state: u32, to_state: u32, event: String, probability: f32) -> PyResult<Self> {
        core(pdviewx::TrajectoryBranch::new(
            from_state,
            to_state,
            event,
            probability,
