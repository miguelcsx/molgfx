...
impl From<PyCameraEasing> for pdviewx::CameraEasing {
...
pub(crate) struct PyCameraKeyframe(pdviewx::CameraKeyframe);

#[pymethods]
impl PyCameraKeyframe {
    #[new]
    fn new(time_seconds: f64, camera: PyCamera) -> PyResult<Self> {
        core(pdviewx::CameraKeyframe::new(time_seconds, camera.inner)).map(Self)
    }
...
#[derive(Clone, Debug)]
pub(crate) struct PyCameraPath(pdviewx::CameraPath);
...
#[derive(Clone, Debug)]
pub(crate) struct PyCameraBookmark(pdviewx::CameraBookmark);

#[pymethods]
impl PyCameraBookmark {
    #[new]
    fn new(label: String, time_seconds: f64, camera: PyCamera) -> PyResult<Self> {
        core(pdviewx::CameraBookmark::new(
            label,
            time_seconds,
            camera.inner,
        ))
    ...
    fn from_json(source: &str) -> PyResult<Self> {
        core(pdviewx::CameraBookmark::from_json(source)).map(Self)
    }
...
impl From<PyPlaybackMode> for pdviewx::PlaybackMode {
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyTimeWarp(pub(crate) pdviewx::TimeWarp);
...
#[derive(Debug)]
pub(crate) struct PyTimeline(pub(crate) pdviewx::Timeline);

#[pymethods]
impl PyTimeline {
    #[new]
    fn new() -> Self {
        Self(pdviewx::Timeline::new())
    }
    ...
    fn bind_vector_attribute_from_numpy(
        ...
        core(self.0.bind_attribute(
            &mut scene.inner,
            attribute.0,
            pdviewx::AttributeValues::Vector(Arc::from(start)),
            pdviewx::AttributeValues::Vector(Arc::from(end)),
            warp.0,
        ))
        .map(Into::into)
    }
...
fn rigid_frames(
...
) -> PyResult<Vec<pdviewx::RigidInstance>> {
    ...
    for row in 0..count {
        frames.push(core(pdviewx::RigidInstance::new(
            pdviewx::Vec3::from_array([
                translations[row * 3],
                translations[row * 3 + 1],
                translations[row * 3 + 2],
            ]),
            pdviewx::Quat::from_array([
                orientations[row * 4],
