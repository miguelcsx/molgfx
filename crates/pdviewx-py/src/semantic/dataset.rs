...
pub(crate) struct PyChunkFootprint(pub(crate) pdviewx::ChunkFootprint);
...
impl From<PyPayloadKind> for pdviewx::PayloadKind {
...
impl From<pdviewx::PayloadKind> for PyPayloadKind {
    fn from(value: pdviewx::PayloadKind) -> Self {
        match value {
            pdviewx::PayloadKind::Structure => Self::Structure,
            pdviewx::PayloadKind::BondTopology => Self::BondTopology,
            pdviewx::PayloadKind::ScalarProperty => Self::ScalarProperty,
            pdviewx::PayloadKind::Trajectory => Self::Trajectory,
            pdviewx::PayloadKind::VolumeBrick => Self::VolumeBrick,
            pdviewx::PayloadKind::LabelBrick => Self::LabelBrick,
            pdviewx::PayloadKind::Mesh => Self::Mesh,
            pdviewx::PayloadKind::Proxy => Self::Proxy,
            pdviewx::PayloadKind::PointBatch => Self::PointBatch,
            pdviewx::PayloadKind::InstanceBatch => Self::InstanceBatch,
            pdviewx::PayloadKind::RelationBatch => Self::RelationBatch,
            pdviewx::PayloadKind::Attribute => Self::Attribute,
        }
    }
}
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyChunkSpan(pub(crate) pdviewx::ChunkSpan);

#[pymethods]
impl PyChunkSpan {
    #[new]
    fn new(first: PyLogicalRow, row_count: u32) -> PyResult<Self> {
        dataset(pdviewx::ChunkSpan::new(first.0, row_count)).map(Self)
    }
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyChunkBounds(pub(crate) pdviewx::ChunkBounds);

#[pymethods]
impl PyChunkBounds {
    #[new]
    fn new(min: [f32; 3], max: [f32; 3]) -> PyResult<Self> {
        dataset(pdviewx::ChunkBounds::new(min, max)).map(Self)
    }
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyChunkDescriptor(pub(crate) pdviewx::ChunkDescriptor);
...
#[derive(Clone, Debug)]
pub(crate) struct PyDatasetCatalog(pub(crate) pdviewx::DatasetCatalog);
...
impl PyDatasetCatalog {
    ...
    fn from_descriptors(
        ...
        dataset(pdviewx::DatasetCatalog::new(dataset_id.0, descriptors)).map(Self)
    }
