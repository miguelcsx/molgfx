...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) struct PyLodClusterKey(pub(crate) pdviewx::LodClusterKey);
...
pub(crate) struct PyLodFrame(pub(crate) pdviewx::LodFrame);
...
#[derive(Clone, Debug)]
pub(crate) struct PyLodIndex(pub(crate) pdviewx::LodIndex);

#[pymethods]
impl PyLodIndex {
    #[new]
    fn new(scene: PyRef<'_, PyScene>) -> Self {
        Self(pdviewx::LodIndex::from_scene(&scene.inner))
    }
    #[getter]
    ...
    fn select_into(
        ...
        let policy = match policy {
            ...
            None => pdviewx::LodPolicy::default(),
        };
        self.0.select_into(
            &camera.inner,
...
#[derive(Clone, Debug, Default)]
pub(crate) struct PyLodScene(pub(crate) pdviewx::LodScene);
...
pub(crate) struct PyStreamPlanner {
    inner: pdviewx::StreamPlanner,
    requests: Vec<pdviewx::ChunkRequest>,
}
...
impl PyStreamPlanner {
    ...
    fn new(budget: Option<PyStreamingBudget>) -> Self {
        let budget = match budget {
            Some(value) => value.0,
            None => pdviewx::StreamingBudget::default(),
        };
        Self {
            inner: pdviewx::StreamPlanner::new(budget),
            requests: Vec::new(),
        }
    }
    #[getter]
...
impl PyStreamPlanner {
    fn copy_request_arrays(
        ...
        for row in 0..length {
            self.requests.push(pdviewx::ChunkRequest {
                key: pdviewx::ChunkKey {
...
fn lod_level(code: u8) -> PyResult<pdviewx::LodLevel> {
    match code {
        0 => Ok(pdviewx::LodLevel::Atom),
        1 => Ok(pdviewx::LodLevel::Residue),
        2 => Ok(pdviewx::LodLevel::SecondaryStructure),
        3 => Ok(pdviewx::LodLevel::Domain),
