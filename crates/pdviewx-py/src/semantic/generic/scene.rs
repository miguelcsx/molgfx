...
impl PyScene {
    ...
    fn compose_focus(
        ...
        composition(pdviewx::GenericCompositionScene::compose_focus(
            &mut self.inner,
            pdviewx::FocusLayer {
                domain: domain.0,
                emphasis: emphasis.0,
    ...
    fn compose_difference(
        ...
        let layers = domains
            .into_iter()
            .zip(deltas)
            .map(|(domain, delta)| pdviewx::DifferenceLayer {
                domain: domain.0,
                delta: delta.0,
            })
            .collect::<Vec<_>>();
        composition(pdviewx::GenericCompositionScene::compose_difference(
            &mut self.inner,
            &layers,
            pdviewx::DifferenceCompositionStyle {
                context_threshold,
                emphasis_threshold,
                context_opacity,
                ramp: ramp.0,
    ...
    fn compose_ensemble(
        ...
        let layers = domains
            ...
            .zip(weights)
            .zip(colors)
            .map(|((domain, weight), color)| pdviewx::EnsembleLayer {
                domain: domain.0,
                weight,
                color: color.0,
            })
            .collect::<Vec<_>>();
        composition(pdviewx::GenericCompositionScene::compose_ensemble(
            &mut self.inner,
            &layers,
            pdviewx::EnsembleCompositionStyle {
                dominant_opacity,
                alternate_opacity,
                minimum_opacity,
                order,
...
fn composition<T>(result: Result<T, pdviewx::CompositionError>) -> PyResult<T> {
