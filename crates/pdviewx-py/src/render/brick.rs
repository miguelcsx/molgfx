...
pub(crate) struct PyBrickId(pub(crate) pdviewx::BrickId);

#[pymethods]
impl PyBrickId {
    #[new]
    fn new(value: u64) -> Self {
        Self(pdviewx::BrickId::new(value))
    }
...
pub(crate) struct PyBrickAddress(pub(crate) pdviewx::BrickAddress);

#[pymethods]
impl PyBrickAddress {
    #[new]
    fn new(origin: [u64; 3], mip: u16) -> Self {
        Self(pdviewx::BrickAddress { origin, mip })
    }
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) struct PyDirtyGeneration(pub(crate) pdviewx::DirtyGeneration);

#[pymethods]
impl PyDirtyGeneration {
    #[new]
    fn new(value: u64) -> Self {
        Self(pdviewx::DirtyGeneration::new(value))
    }
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyBrickValueRange(pub(crate) pdviewx::BrickValueRange);

#[pymethods]
impl PyBrickValueRange {
    #[staticmethod]
    fn scalar(min: f32, max: f32) -> Self {
        Self(pdviewx::BrickValueRange::Scalar { min, max })
    }

    #[staticmethod]
    fn segmentation(min: u32, max: u32) -> Self {
        Self(pdviewx::BrickValueRange::Segmentation { min, max })
    }

    #[staticmethod]
    fn occupancy(has_empty: bool, has_occupied: bool) -> Self {
        Self(pdviewx::BrickValueRange::Occupancy {
            has_empty,
            has_occupied,
        })
    }
    ...
    fn kind(&self) -> &'static str {
        match self.0 {
            pdviewx::BrickValueRange::Scalar { .. } => "scalar",
            pdviewx::BrickValueRange::Segmentation { .. } => "segmentation",
            pdviewx::BrickValueRange::Occupancy { .. } => "occupancy",
        }
    }
}
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyBrickShape(pub(crate) pdviewx::BrickShape);

#[pymethods]
impl PyBrickShape {
    #[new]
    fn new(stored: [u16; 3], halo: u16) -> PyResult<Self> {
        dataset(pdviewx::BrickShape::new(stored, halo)).map(Self)
    }
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyBrickMetadata(pub(crate) pdviewx::BrickMetadata);
...
impl PyBrickMetadata {
    ...
    fn new(
        ...
        dataset(pdviewx::BrickMetadata::new(
            id.0,
            address.0,
            shape.0,
            value_range.0,
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyBrickDescriptor(pub(crate) pdviewx::BrickDescriptor);

#[pymethods]
impl PyBrickDescriptor {
    #[new]
    fn new(chunk: PyChunkId, metadata: PyBrickMetadata) -> Self {
        Self(pdviewx::BrickDescriptor {
            chunk: chunk.0,
            metadata: metadata.0,
        })
    }
...
#[derive(Clone, Debug)]
pub(crate) struct PyBrickCatalog(pub(crate) pdviewx::BrickCatalog);
...
impl PyBrickCatalog {
    ...
    fn from_descriptors(
        ...
        dataset(pdviewx::BrickCatalog::new(
            dataset_id.0,
            logical_extent,
            voxel_bytes,
            descriptors,
