...
impl PyScene {
    ...
    fn add_point_batch_from_numpy(
        ...
        let glyph = if sphere {
            pdviewx::PointGlyph::Sphere
        } else {
            pdviewx::PointGlyph::Disc
        };
        let style = pdviewx::PointStyle {
            radius,
            color: pdviewx::Rgba8::new(color.0, color.1, color.2, color.3),
        };
        core(pdviewx::PointBatch::new(
            Arc::from(positions),
            rows,
            glyph,
            style,
    ...
    fn add_instance_batch_from_numpy(
        ...
        for ((translation, orientation), scale) in translations
            ...
            .zip(scales)
        {
            transforms.push(core(pdviewx::RigidInstance::new(
                pdviewx::Vec3::new(translation[0], translation[1], translation[2]),
        ...
        core(pdviewx::InstanceBatch::new(
            Arc::clone(&template.0),
            Arc::from(transforms),
            rows,
        ))
        .map(|batch| {
            batch.with_style(pdviewx::InstanceStyle {
                color: pdviewx::Rgba8::new(color.0, color.1, color.2, color.3),
            })
        })
