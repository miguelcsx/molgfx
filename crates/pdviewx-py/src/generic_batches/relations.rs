...
impl PyScene {
    ...
    fn add_world_relations_from_numpy(
        ...
        for (start, end) in starts.into_iter().zip(ends) {
            relations.push(pdviewx::Relation {
                start: core(pdviewx::SpatialAnchor::world(pdviewx::Vec3::from_array(
                    start,
                )))?,
                end: core(pdviewx::SpatialAnchor::world(pdviewx::Vec3::from_array(
                    end,
                )))?,
            });
        }
    ...
    fn add_entity_relations_from_numpy(
        ...
        for (&start, &end) in start_rows.iter().zip(end_rows) {
            relations.push(pdviewx::Relation {
                start: core(pdviewx::SpatialAnchor::entity(pdviewx::RowEntityRef::new(
                    start_domain.0,
                    start,
                )))?,
                end: core(pdviewx::SpatialAnchor::entity(pdviewx::RowEntityRef::new(
                    end_domain.0,
                    end,
                )))?,
            });
