...
#[derive(Clone, Debug)]
pub(crate) struct PyAtomSelection(pub(crate) pdviewx::AtomSelection);

#[pymethods]
impl PyAtomSelection {
    #[staticmethod]
    fn empty() -> Self {
        Self(pdviewx::AtomSelection::Empty)
    }

    #[staticmethod]
    fn all() -> Self {
        Self(pdviewx::AtomSelection::All)
    }

    #[staticmethod]
    fn range(start: u32, end: u32) -> Self {
        Self(pdviewx::AtomSelection::Range(start..end))
    }

    #[staticmethod]
    fn copy_sparse(rows: Vec<u32>) -> Self {
        Self(pdviewx::AtomSelection::Sparse(rows))
    }
