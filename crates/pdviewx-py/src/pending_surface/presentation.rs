...
impl From<PyFaceVisibility> for pdviewx::FaceVisibility {
...
impl From<pdviewx::FaceVisibility> for PyFaceVisibility {
    fn from(value: pdviewx::FaceVisibility) -> Self {
        match value {
            pdviewx::FaceVisibility::DoubleSided => Self::DoubleSided,
            pdviewx::FaceVisibility::FrontOnly => Self::FrontOnly,
            pdviewx::FaceVisibility::BackOnly => Self::BackOnly,
        }
    }
}
...
pub(crate) struct PySurfaceComponentPolicy(pub(crate) pdviewx::SurfaceComponentPolicy);

#[pymethods]
impl PySurfaceComponentPolicy {
    #[new]
    fn new() -> Self {
        Self(pdviewx::SurfaceComponentPolicy::keep_all())
    }
    ...
    fn minimum_area(area: f64) -> PyResult<Self> {
        pdviewx::SurfaceComponentPolicy::minimum_area(area)
            .map(Self)
    ...
    fn minimum_volume(volume: f64) -> PyResult<Self> {
        pdviewx::SurfaceComponentPolicy::minimum_volume(volume)
            .map(Self)
            .map_err(|error| crate::error::value(error.to_string()))
    }
    ...
    fn minimum_voxels(voxels: u64) -> PyResult<Self> {
        pdviewx::SurfaceComponentPolicy::minimum_voxels(voxels)
            .map(Self)
            .map_err(|error| crate::error::value(error.to_string()))
    }
...
#[derive(Clone, Debug)]
pub(crate) struct PyPropertyLegend(pub(crate) pdviewx::PropertyLegend);
