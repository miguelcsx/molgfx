...
#[derive(Clone, Debug)]
pub(crate) struct PyRepresentationInput(pub(crate) pdviewx::RepresentationInput);

#[pymethods]
impl PyRepresentationInput {
    #[staticmethod]
    fn stored(selection: PySelectionHandle) -> Self {
        Self(pdviewx::RepresentationInput::Stored(selection.0))
    }

    #[staticmethod]
    fn query(query: PySelect) -> Self {
        Self(pdviewx::RepresentationInput::Query(query.0))
    }

    #[staticmethod]
    fn source(source: String) -> Self {
        Self(pdviewx::RepresentationInput::Source(
    ...
    fn volume(volume: PyVolumeHandle) -> Self {
        Self(pdviewx::RepresentationInput::Volume(volume.0))
    }

    #[staticmethod]
    fn segmentation(segmentation: PySegmentationHandle) -> Self {
        Self(pdviewx::RepresentationInput::Segmentation(segmentation.0))
    }

    #[getter]
    fn kind(&self) -> &'static str {
        match &self.0 {
            pdviewx::RepresentationInput::Stored(_) => "stored",
            pdviewx::RepresentationInput::Query(_) => "query",
            pdviewx::RepresentationInput::Source(_) => "source",
            pdviewx::RepresentationInput::Volume(_) => "volume",
            pdviewx::RepresentationInput::Segmentation(_) => "segmentation",
        }
    }
}
...
pub(crate) struct PyRepresentationParams(pub(crate) pdviewx::RepresentationParams);
...
impl PyRepresentationParams {
    ...
    fn new(
        ...
        Self(pdviewx::RepresentationParams {
            radius_scale,
            bond_radius,
            probe_radius,
            gaussian_sigma,
            ...
            surface_components: surface_components
                .map_or_else(pdviewx::SurfaceComponentPolicy::keep_all, |policy| policy.0),
            surface_pattern_spacing,
            surface_pattern_width_pixels,
            ribbon_width,
            tube_radius,
            tube_radius_mapping: tube_radius_mapping
                .map_or(pdviewx::TubeRadiusMapping::Constant, |value| value.0),
            point_size_pixels,
            line_width_pixels,
        })
    }
    ...
    #[staticmethod]
    fn default() -> Self {
        Self(pdviewx::RepresentationParams::default())
    }
...
#[derive(Clone, Debug)]
pub(crate) struct PyRepresentationConfig(pub(crate) pdviewx::RepresentationConfig);

#[pymethods]
impl PyRepresentationConfig {
    #[new]
    fn new(kind: PyRepresentationKind) -> Self {
        Self(pdviewx::RepresentationConfig::new(kind.into()))
    }
