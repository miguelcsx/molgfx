...
pub(crate) struct PyVisualInputs(pdviewx::VisualInputs);
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyVisualEvaluation(pdviewx::VisualEvaluation);
...
#[derive(Clone, Debug)]
pub(crate) struct PyVisualProgram(pub(crate) pdviewx::VisualProgram);
...
#[derive(Clone, Debug)]
pub(crate) struct PyVisualStyle(pub(crate) pdviewx::VisualStyle);

#[pymethods]
impl PyVisualStyle {
    #[new]
    fn new(program: PyVisualProgram) -> Self {
        Self(pdviewx::VisualStyle::new(program.0))
    }

    #[staticmethod]
    fn pulse(color: PyRgba8, cycles_per_second: f32, minimum: f32, maximum: f32) -> PyResult<Self> {
        visual(pdviewx::VisualStyle::pulse(
            color.0,
            cycles_per_second,
            minimum,
            maximum,
...
pub(crate) struct PyVisualProgramBuilder {
    inner: Option<pdviewx::VisualProgramBuilder>,
}

impl PyVisualProgramBuilder {
    fn builder(&mut self) -> PyResult<&mut pdviewx::VisualProgramBuilder> {
        self.inner
            .as_mut()
