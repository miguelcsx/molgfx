...
impl PyScene {
    ...
    fn occupancy_mask_from_python(
        ...
        object: &Bound<'_, PyAny>,
    ) -> PyResult<pdviewx::AtomSelection> {
...
impl PyScene {
    ...
    fn from_manifest_json(
        ...
        let segmentations = segmentations
            ...
            .map(|value| value.0)
            .collect::<Vec<_>>();
        let manifest = manifest(pdviewx::read_manifest(source.as_bytes()))?;
        core(pdviewx::Scene::from_description(
            &manifest.scene,
            pdviewx::SceneDescriptionSources {
    ...
    fn copy_manifest_json<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyBytes>> {
        ...
        manifest(pdviewx::write_manifest(&mut encoded, &manifest_value))?;
