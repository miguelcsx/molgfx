...
impl From<PyParticleShape> for pdviewx::ParticleShape {
...
impl From<PyMeshTopology> for pdviewx::MeshTopology {
...
impl From<PyGuideCap> for pdviewx::GuideCap {
...
pub(crate) struct PyGuideStyle(pub(crate) pdviewx::GuideStyle);
...
impl PyScene {
    fn copy_mesh_from_numpy(
        ...
        for row in 0..count {
            vertices.push(pdviewx::MeshVertex {
                position: pdviewx::Vec3::from_slice(&positions[row * 3..row * 3 + 3]),
                normal: pdviewx::Vec3::from_slice(&normals[row * 3..row * 3 + 3]),
                color: pdviewx::Rgba8::new(
                    colors[row * 4],
                    colors[row * 4 + 1],
                    colors[row * 4 + 2],
                    colors[row * 4 + 3],
        ...
        let mesh = core(pdviewx::Mesh::new(
            owner.0,
            vertices,
            indices.to_vec(),
            material.0,
    ...
    fn copy_particles_from_numpy(
        ...
        for row in 0..count {
            let particle = core(pdviewx::Particle::new(
                owner.0,
                pdviewx::Vec3::from_slice(&centers[row * 3..row * 3 + 3]),
                pdviewx::Quat::from_xyzw(
                    orientations[row * 4],
                    orientations[row * 4 + 1],
                    orientations[row * 4 + 2],
                    orientations[row * 4 + 3],
                ),
                pdviewx::Vec3::from_slice(&sizes[row * 3..row * 3 + 3]),
                shape.into(),
                pdviewx::Rgba8::new(
                    colors[row * 4],
                    colors[row * 4 + 1],
            ...
            primitives.push(pdviewx::Primitive::Particle(particle));
        }
    ...
    fn copy_polyline_from_numpy(
        ...
        let points = points
            .chunks_exact(3)
            .map(pdviewx::Vec3::from_slice)
            .collect::<Vec<_>>();
        let kind = if closed {
            pdviewx::PolylineKind::Closed
        } else {
            pdviewx::PolylineKind::Open
        };
