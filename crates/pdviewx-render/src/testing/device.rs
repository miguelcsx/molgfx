...
impl pdviewx_gpu::Device for MockDevice {
    ...
    fn create_bind_group(&self, desc: &BindGroupDesc<'_, Self>) -> u32 {
        if let Ok(mut bindings) = self.log.buffer_bindings.lock() {
            for entry in desc.entries {
                match entry {
                    pdviewx_gpu::BindGroupEntry::BufferRange {
                        binding,
                        buffer,
                        offset,
                        size,
                    } => bindings.push((desc.label, *binding, buffer.id, *offset, *size)),
                    pdviewx_gpu::BindGroupEntry::Buffer { binding, buffer } => {
                        bindings.push((desc.label, *binding, buffer.id, 0, 0));
                    }
                    pdviewx_gpu::BindGroupEntry::Texture { .. }
                    | pdviewx_gpu::BindGroupEntry::Sampler { .. } => {}
                }
            }
        }
        0
    ...
    fn create_blas(&self, _desc: &pdviewx_gpu::BlasDesc<'_>) -> Result<MockBlas, GpuError> {
    ...
    fn create_tlas(&self, desc: &pdviewx_gpu::TlasDesc) -> Result<MockTlas, GpuError> {
        self.ray_query_limits()?;
        fail_mock_ray_query(&self.log)?;
        self.log
            .acceleration_allocations
    ...
    fn create_ray_query_bind_group_layout(
        &self,
        _desc: &pdviewx_gpu::RayQueryBindGroupLayoutDesc<'_>,
    ...
    fn create_ray_query_bind_group(
        &self,
        _desc: &pdviewx_gpu::RayQueryBindGroupDesc<'_, Self>,
    ) -> Result<u32, GpuError> {
        self.ray_query_limits()?;
        fail_mock_ray_query(&self.log)?;
        Ok(0)
