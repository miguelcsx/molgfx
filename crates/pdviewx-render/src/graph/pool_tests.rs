...
fn the_physical_pool_is_stable_across_frames_at_one_size() {
    ...
    let order = vec![0, 1];
    let plan = plan_aliases(&resources, &passes, &order);
    let opened = match <MockDevice as pdviewx_gpu::Device>::open_blocking(
        &pdviewx_gpu::DeviceDesc::default(),
        None,
    ) {
        Ok(opened) => opened,
...
fn inactive_resources_have_no_allocation_even_if_marked_persistent() {
    ...
    let opened = MockDevice::open_blocking(&pdviewx_gpu::DeviceDesc::default(), None)
