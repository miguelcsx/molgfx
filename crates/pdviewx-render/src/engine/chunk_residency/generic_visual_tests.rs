...
fn paged_relation_visual_reads_one_native_resident_column() {
    ...
    let mut scene = pdviewx_core::Scene::new();
    engine
...
fn assert_attribute_advance(
    ...
    scene: &pdviewx_core::Scene,
    attribute: pdviewx_core::ResidencyTicket,
    start: pdviewx_core::ResidencyTicket,
    end: pdviewx_core::ResidencyTicket,
    buffers: usize,
) {
    let geometry_writes = relation_geometry_write_count(engine);
    let visual_writes = relation_write_count(engine);
...
fn animated_relation_visual(
    attribute_ticket: pdviewx_core::ResidencyTicket,
