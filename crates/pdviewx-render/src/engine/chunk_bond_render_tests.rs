...
fn missing_cross_chunk_endpoint_remains_blocked_and_nonresident() {
    ...
    assert_eq!(
        engine
            ...
            .snapshot(bond.key)
            .phase,
        pdviewx_core::ResidencyPhase::Uploading
    );
}
...
fn bond_delivered_before_atoms_retries_without_redelivery_or_restage() {
    ...
    assert_eq!(
        engine
            ...
            .snapshot(bond.key)
            .phase,
        pdviewx_core::ResidencyPhase::Resident
    );
}
...
fn fixtures() -> (Fixture, Fixture, Fixture) {
    ...
    let atom_bridge = pdviewx_core::ProviderDatasetBridge::new(atom_provider);
    ...
    let bridge = pdviewx_core::ProviderDatasetBridge::new(bond_provider);
    let footprint = ChunkFootprint::new(32, 32, 256, 256);
    let data = bridge
        .bond_chunk(
            bond,
            pdviewx_core::ChunkBounds::new([-1.0; 3], [4.0; 3])
...
fn structure_fixture(
    bridge: &pdviewx_core::ProviderDatasetBridge,
...
fn request_ticket(fixture: &Fixture) -> pdviewx_core::ResidencyTicket {
    let mut engine = engine();
    let mut output = ResidencyOutput::default();
    request_and_deliver(&mut engine, fixture.clone(), &mut output).0
}

fn licorice(ticket: pdviewx_core::ResidencyTicket, id: u64) -> BondChunkPlacement {
...
fn assert_bond_pick(engine: &mut Engine<MockDevice>, page: Option<u32>) {
    ...
    assert_eq!(identity.kind(), pdviewx_core::EntityKind::Bond);
