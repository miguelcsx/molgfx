...
fn profiling_target_uses_the_render_pipeline_format() {
    ...
    let mut profiler = GpuProfiler::new(&device, pdviewx_gpu::TextureFormat::Bgra8Unorm)
    ...
    assert!(formats.contains(&("profiling target", pdviewx_gpu::TextureFormat::Bgra8Unorm)));
}
