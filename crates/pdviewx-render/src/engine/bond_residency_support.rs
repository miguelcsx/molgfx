...
use pdviewx_core::{ChunkPayload, ChunkSpan, DatasetId, LogicalRow};
    ...
    for local in 0..bonds.descriptor().rows() {
        ...
        for endpoint in [value.atom_a, value.atom_b] {
            ...
            if endpoint.dataset().get() == dataset.get()
                && span.local_row(pdviewx_core::ChunkId::new(0), row).is_ok()
            {
                return Ok(true);
            }
        }
...
impl<D: pdviewx_gpu::Device> super::BondGpuResidency<D> {
    ...
    pub(super) fn ensure_buffer(
        ...
        if !self.buffer_allocated {
            self.buffer = device.create_buffer(&pdviewx_gpu::BufferDesc {
                label: "resident provider bonds",
                size: self.arena.capacity_bytes(),
                usage: pdviewx_gpu::BufferUsage::STORAGE.union(pdviewx_gpu::BufferUsage::COPY_DST),
            })?;
