...
use pdviewx_core::{AtomSelection, Scene, SelectionHandle, StructureHandle};
use pdviewx_gpu::Device;
use pdviewx_math::{Camera, Mat4, Vec3};
...
impl FocusTracker {
    ...
    fn selection_centroid(
        ...
        for (structure, placed) in scene.structures() {
            ...
            let alpha = placed
                .trajectory()
                .map_or(0.0, pdviewx_core::TrajectorySegment::interpolation);
...
fn reduce_structure(
    structure: StructureHandle,
    placed: &pdviewx_core::PlacedStructure,
    rows: &AtomSelection,
