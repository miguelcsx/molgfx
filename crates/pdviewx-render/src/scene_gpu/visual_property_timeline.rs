...
fn paged_timeline_config(
    ...
    let word_offset = |bytes: u64| {
        ...
        u32::try_from(bytes / 4).map_err(|_| {
            RenderError::from(pdviewx_gpu::GpuError::LimitExceeded {
                resource: "paged attribute timeline offset",
                limit: u64::from(u32::MAX) * 4,
            })
        })
...
fn program_consumers(
    ...
    let cull = [
        pdviewx_core::VisualOutput::Visibility,
        pdviewx_core::VisualOutput::RadiusScale,
        pdviewx_core::VisualOutput::WidthScale,
        pdviewx_core::VisualOutput::PositionOffset,
    ]
    .iter()
    .any(|output| program.output_register(*output).is_some());
    let shading = [
        pdviewx_core::VisualOutput::BaseColor,
        pdviewx_core::VisualOutput::Opacity,
        pdviewx_core::VisualOutput::Emission,
        pdviewx_core::VisualOutput::Roughness,
        pdviewx_core::VisualOutput::Specular,
        pdviewx_core::VisualOutput::MaterialStrength,
    ]
    .iter()
    .any(|output| program.output_register(*output).is_some());
    usize::from(cull)
