...
use super::super::volume_slot::{GpuVolumeResource, VolumeSync};
use super::GpuScene;
...
impl<D: Device> GpuScene<D> {
    pub(super) fn sync_volume_resources(
        ...
        for resource in &mut self.volume_resources {
            let Some(volume) = scene.volume(resource.handle) else {
                continue;
            };
    ...
    pub(super) fn sync_volume_slots(
        ...
        for slot in &mut self.volume_slots {
            ...
            let Some(volume) = scene.volume(volume_handle) else {
                continue;
            };
            ...
            let Some((empty_space_minimum_view, empty_space_maximum_view)) = self
                .volume_resources
                .iter()
                .find(|resource| resource.handle == volume_handle)
            ...
            changed |= slot.sync(&VolumeSync {
                ...
                volume_handle,
                volume_view,
                empty_space_minimum_view,
                empty_space_maximum_view,
