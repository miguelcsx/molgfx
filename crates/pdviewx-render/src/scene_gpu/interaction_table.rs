...
enum RelationVisualSource {
    ...
    Paged(pdviewx_core::ChunkOccurrenceId),
}
...
pub(super) struct GpuInteractions<D: Device> {
    ...
    paged_visuals: BTreeMap<pdviewx_core::ChunkOccurrenceId, PagedRelationVisualState<D>>,
        ...
        if data_changed {
            ...
            if legacy_count != 0 {
                self.visual_plans.push(RelationVisualPlan {
                    ...
                    first: 0,
                    count: legacy_count,
                    color: pdviewx_math::Rgba8::WHITE,
                    opacity: 1.0,
                });
            }
            ...
            .filter(|(_, batch)| batch.visible())
        {
            ...
            for (logical, relation) in batch.relations().iter().copied().enumerate() {
                ...
                if matches!(
                    ...
                    (
                        pdviewx_core::SpatialAnchor::World(_),
                        pdviewx_core::SpatialAnchor::World(_)
                    )
                ) {
