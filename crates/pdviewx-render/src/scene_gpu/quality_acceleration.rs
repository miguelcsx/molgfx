...
use pdviewx_math::{Aabb, Bvh, BvhBuildScratch, Vec3};
...
pub(super) fn hierarchy_counts(hierarchy: &Bvh) -> Result<HierarchyCounts, RenderError> {
    ...
    let nodes =
        u32::try_from(hierarchy.nodes.len()).map_err(|_| pdviewx_gpu::GpuError::LimitExceeded {
            resource: "quality hierarchy node count",
            limit: u64::from(u32::MAX),
        })?;
    let indices = u32::try_from(index_count).map_err(|_| pdviewx_gpu::GpuError::LimitExceeded {
        resource: "quality hierarchy index count",
        limit: u64::from(u32::MAX),
    })?;
