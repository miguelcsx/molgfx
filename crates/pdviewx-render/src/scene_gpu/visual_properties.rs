...
use pdviewx_core::{RowDomain, Scene, StructureHandle, VisualAttributeRef, VisualStyle};
...
impl<D: Device> VisualPropertyTable<D> {
    ...
    fn refresh_plan(
        ...
        for &reference in &self.planned_handles {
            ...
            let length =
                u32::try_from(length).map_err(|_| pdviewx_gpu::GpuError::LimitExceeded {
...
fn upload_temporal_column<D: Device>(
    ...
    start: &pdviewx_core::AttributeValues,
    end: &pdviewx_core::AttributeValues,
    alpha: f32,
) {
