...
use molgfx_core::{BrickAddress, BrickCatalog, BrickDescriptor, BrickId, BrickValueRange};
use molgfx_gpu::{
...
use molgfx_semantic::BrickWorkingSet;
...
impl<D: Device> GpuBrickAtlas<D> {
    ...
    pub fn request_eviction(
        ...
        let Some(page) = self.working_set.get(brick).copied() else {
            return Err(molgfx_semantic::BrickWorkingSetError::NotResident { brick }.into());
        };
        let Some(pending_index) = self
            .pending
            .iter()
