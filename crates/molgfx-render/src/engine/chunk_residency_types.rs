...
use molgfx_core::{ChunkDomainRef, PayloadKind, ResidencyTicket};
use thiserror::Error;
...
pub struct ChunkResidencyMetrics {
    ...
    pub arena: molgfx_gpu::ArenaMetrics,
    /// Fixed upload-ring counters.
    pub uploads: molgfx_gpu::UploadMetrics,
...
pub enum ChunkResidencyError {
    ...
    #[error(transparent)]
    Host(#[from] molgfx_core::HostWorkingSetError),
    ...
    #[error(transparent)]
    Arena(#[from] molgfx_gpu::ArenaError),
    /// Upload staging applied explicit backpressure.
    #[error(transparent)]
    Upload(#[from] molgfx_gpu::UploadBackpressure),
