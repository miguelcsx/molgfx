...
pub struct WebDifferenceComposition {
    layers: Vec<pdviewx::DifferenceLayer>,
    style: pdviewx::DifferenceCompositionStyle,
}
...
impl WebDifferenceComposition {
    ...
    pub fn add_layer(&mut self, domain: &WebRowDomain, delta: &WebAttributeHandle) {
        self.layers.push(pdviewx::DifferenceLayer {
            domain: domain.inner,
            delta: delta.inner,
        });
    }
    ...
    pub fn apply(&self, scene: &mut WebScene) -> Result<WebGenericCompositionView, JsValue> {
        pdviewx::GenericCompositionScene::compose_difference(
            &mut scene.inner,
            &self.layers,
            self.style,
        )
