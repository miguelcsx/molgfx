...
pub struct WebEnsembleComposition {
    layers: Vec<pdviewx::EnsembleLayer>,
    style: pdviewx::EnsembleCompositionStyle,
}
...
impl WebEnsembleComposition {
    ...
    pub fn add_layer(
        ...
        self.layers.push(pdviewx::EnsembleLayer {
            domain: domain.inner,
            weight,
    ...
    pub fn apply(&self, scene: &mut WebScene) -> Result<WebGenericCompositionView, JsValue> {
        pdviewx::GenericCompositionScene::compose_ensemble(
            &mut scene.inner,
            &self.layers,
            self.style,
        )
