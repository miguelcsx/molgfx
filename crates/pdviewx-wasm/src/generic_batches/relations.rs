...
impl WebScene {
    ...
    pub fn add_world_relations(
        ...
        for (start, end) in starts.into_iter().zip(ends) {
            relations.push(pdviewx::Relation {
                start: pdviewx::SpatialAnchor::world(pdviewx::Vec3::from_array(start))
                    .map_err(core_error)?,
                end: pdviewx::SpatialAnchor::world(pdviewx::Vec3::from_array(end))
                    .map_err(core_error)?,
            });
        }
        insert_relations(
    ...
    pub fn add_entity_relations(
        ...
        for (start, end) in start_rows.into_iter().zip(end_rows) {
            relations.push(pdviewx::Relation {
                start: pdviewx::SpatialAnchor::entity(pdviewx::RowEntityRef::new(
                    start_domain.inner,
                    start,
                ))
                .map_err(core_error)?,
                end: pdviewx::SpatialAnchor::entity(pdviewx::RowEntityRef::new(
                    end_domain.inner,
                    end,
                ))
                .map_err(core_error)?,
...
fn insert_relations(
    ...
    let batch = pdviewx::RelationBatch::new(
        Arc::from(relations),
        rows,
        pdviewx::RelationStyle {
            ...
            pattern: pdviewx::RelationPattern::Solid,
