...
impl WebScene {
    ...
    pub fn add_point_batch(
        ...
        let batch = pdviewx::PointBatch::new(
            Arc::from(positions),
            rows,
            glyph,
            pdviewx::PointStyle { radius, color },
        )
        .map_err(core_error)?;
    ...
    pub fn add_instance_batch(
        ...
        let batch =
            pdviewx::InstanceBatch::new(Arc::clone(&template.inner), Arc::from(transforms), rows)
                .map_err(core_error)?
                .with_style(pdviewx::InstanceStyle {
                    color: rgba8(&rgba)?,
                });
