...
impl From<PyVolumeRendering> for molgfx::VolumeRendering {
...
impl From<molgfx::VolumeRendering> for PyVolumeRendering {
    fn from(value: molgfx::VolumeRendering) -> Self {
        match value {
            molgfx::VolumeRendering::Direct => Self::Direct,
            molgfx::VolumeRendering::Isosurface => Self::Isosurface,
            molgfx::VolumeRendering::Medium => Self::Medium,
            molgfx::VolumeRendering::Slice => Self::Slice,
            molgfx::VolumeRendering::LiquidSurface => Self::LiquidSurface,
        }
    }
}
...
pub(crate) struct PyVolumeTransferPoint(pub(crate) molgfx::VolumeTransferPoint);

#[pymethods]
impl PyVolumeTransferPoint {
    #[new]
    fn new(value: f32, color: PyRgba8, opacity: f32) -> Self {
        Self(molgfx::VolumeTransferPoint::new(value, color.0, opacity))
    }
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyVolumeTransferFunction(pub(crate) molgfx::VolumeTransferFunction);
...
impl PyVolumeTransferFunction {
    ...
    fn new(points: Vec<PyVolumeTransferPoint>) -> PyResult<Self> {
        ...
        core(molgfx::VolumeTransferFunction::new(&points)).map(Self)
    }

    #[staticmethod]
    fn linear(range: [f32; 2], low: PyRgba8, high: PyRgba8) -> Self {
        Self(molgfx::VolumeTransferFunction::linear(range, low.0, high.0))
    }

    #[staticmethod]
    fn default() -> Self {
        Self(molgfx::VolumeTransferFunction::default())
    }
