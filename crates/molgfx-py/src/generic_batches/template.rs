...
#[derive(Clone, Debug)]
pub(crate) struct PyAnalyticTemplate(pub(super) Arc<molgfx::AnalyticTemplate>);
...
impl PyAnalyticTemplate {
    ...
    fn new(
        ...
        for row in capsule_values.chunks_exact(7) {
            capsules.push(core(molgfx::AnalyticCapsule::new(
        ...
        core(molgfx::AnalyticTemplate::new(
            Arc::from(spheres),
            Arc::from(capsules),
            rows,
        ))
...
impl PyAnalyticTemplate {
    pub(crate) fn native(&self) -> Arc<molgfx::AnalyticTemplate> {
        Arc::clone(&self.0)
    }
}
