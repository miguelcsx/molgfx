...
pub(crate) struct PyVec3(pub(crate) molgfx::Vec3);

#[pymethods]
impl PyVec3 {
    #[new]
    fn new(x: f32, y: f32, z: f32) -> Self {
        Self(molgfx::Vec3::new(x, y, z))
    }

    #[classattr]
    #[pyo3(name = "ZERO")]
    fn zero() -> Self {
        Self(molgfx::Vec3::ZERO)
    }
...
#[derive(Clone, Copy, Debug, PartialEq)]
pub(crate) struct PyQuat(pub(crate) molgfx::Quat);

#[pymethods]
impl PyQuat {
    #[new]
    fn new(x: f32, y: f32, z: f32, w: f32) -> Self {
        Self(molgfx::Quat::from_xyzw(x, y, z, w))
    }

    #[staticmethod]
    fn identity() -> Self {
        Self(molgfx::Quat::IDENTITY)
    }
...
#[derive(Clone, Copy, Debug, PartialEq)]
pub(crate) struct PyMat4(pub(crate) molgfx::Mat4);
...
impl PyMat4 {
    ...
    fn new(values: Vec<f32>) -> PyResult<Self> {
        ...
        Ok(Self(molgfx::Mat4::from_cols_array(&values)))
    }

    #[staticmethod]
    fn identity() -> Self {
        Self(molgfx::Mat4::IDENTITY)
    }
...
pub(crate) struct PyRgba8(pub(crate) molgfx::Rgba8);

impl From<molgfx::Rgba8> for PyRgba8 {
    fn from(value: molgfx::Rgba8) -> Self {
        Self(value)
    }
}
...
impl PyRgba8 {
    ...
    fn new(r: u8, g: u8, b: u8, a: u8) -> Self {
        Self(molgfx::Rgba8::new(r, g, b, a))
    }

    #[staticmethod]
    fn opaque(r: u8, g: u8, b: u8) -> Self {
        Self(molgfx::Rgba8::opaque(r, g, b))
    }

    #[classattr]
    #[pyo3(name = "WHITE")]
    fn white() -> Self {
        Self(molgfx::Rgba8::WHITE)
    }
...
pub(crate) struct PyAabb(pub(crate) molgfx::Aabb);

#[pymethods]
impl PyAabb {
    #[new]
    fn new(min: PyVec3, max: PyVec3) -> Self {
        Self(molgfx::Aabb::new(min.0, max.0))
    }

    #[classattr]
    #[pyo3(name = "EMPTY")]
    fn empty() -> Self {
        Self(molgfx::Aabb::EMPTY)
    }
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyBoundingSphere(pub(crate) molgfx::BoundingSphere);

#[pymethods]
impl PyBoundingSphere {
    #[new]
    fn new(center: PyVec3, radius: f32) -> Self {
        Self(molgfx::BoundingSphere {
            center: center.0,
            radius,
        })
    }
...
#[derive(Clone, Copy, Debug)]
pub(crate) struct PyProjection(pub(crate) molgfx::Projection);
...
impl PyProjection {
    ...
    fn perspective(fov_y: f32, aspect: f32, near: f32, far: f32) -> Self {
        Self(molgfx::Projection::Perspective {
            fov_y,
            aspect,
            near,
            far,
    ...
    fn orthographic(height: f32, aspect: f32, near: f32, far: f32) -> Self {
        Self(molgfx::Projection::Orthographic {
            height,
            aspect,
            near,
            far,
    ...
    fn kind(&self) -> &'static str {
        match self.0 {
            molgfx::Projection::Perspective { .. } => "perspective",
            molgfx::Projection::Orthographic { .. } => "orthographic",
        }
    }
...
impl PyCamera {
    ...
    fn framing(bound: PyBoundingSphere, aspect: f32) -> Self {
        Self {
            inner: molgfx::Camera::framing(&bound.0, aspect),
        }
    }
    ...
    fn framing_aabb(bound: PyAabb, aspect: f32) -> Self {
        Self {
            inner: molgfx::Camera::framing_aabb(&bound.0, aspect),
        }
    }

    #[staticmethod]
    fn look_at(eye: PyVec3, target: PyVec3, up: PyVec3) -> PyMat4 {
        PyMat4(molgfx::Camera::look_at(eye.0, target.0, up.0))
    }
