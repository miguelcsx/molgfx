...
use molgfx::{VisualInputs, VisualProgramBuilder, VisualStyle};
use std::error::Error;
use std::fs::File;
...
fn main() -> Result<(), Box<dyn Error>> {
    let output = std::env::args().nth(1).map_or_else(
        || PathBuf::from("/private/tmp/molgfx-visual-sequence"),
        PathBuf::from,
    );
...
fn visual_style() -> Result<VisualStyle, molgfx::VisualError> {
