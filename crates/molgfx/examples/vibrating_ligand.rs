...
fn profile_animation(
    ...
    scene: &mut Scene,
    camera: &Camera,
    points: molgfx::PointBatchHandle,
...
fn add_atoms(
    scene: &mut Scene,
    positions: Arc<[[f32; 3]]>,
) -> Result<molgfx::PointBatchHandle, Box<dyn Error>> {
...
fn add_bonds(scene: &mut Scene, points: molgfx::PointBatchHandle) -> Result<(), Box<dyn Error>> {
    let domain = RowDomain::Points(points);
    ...
    let relations = pairs
        ...
        .collect::<Result<Vec<_>, molgfx::CoreError>>()?;
